...
class ControllerTest extends FunSuite with TestKitBase with ControllerTesting {
  ...
  test("the controller should respond nicely") {
    assertResponse(200, "Hello!", request(get("/")).value.get.get, "No gentle Hello!...")
    assertResponse(404, "Nobody home at /spam.", request(get("/spam")).value.get.get, "Expected a 404...")
    assertResponse(200, "asdf", request(post("/gringo", "asdf")).value.get.get)
    assertResponse(200, "They are all here!", request(get("/a/b/c/d")).value.get.get, "No one home at /a/b/c/d")
    assertResponse(200, "q/q", request(get("/q/q")).value.get.get, "First match should be returned when multiple actions match.")
    assertResponse(200, "q/:q", request(get("/q/:q")).value.get.get, "'Regexes' are valid paths.")
    assertResponse(200, "spam", request(get("/test/spam")).value.get.get, "PathParams did not work as expected.")
    assertResponse(200, "Hello a and b!", request(get("/hello/a/and/b")).value.get.get, "A more complicated path failed.")
    assertResponse(200, "Your id #300.", request(post("/form/400", "id=300").withHeaders(HttpHeaders.`Content-Type`(ContentType(MediaType.custom("application/x-www-form-urlencoded"))))).value.get.get)
    assertResponse(200, "Your id #400.", request(post("/form/400", "{text:1}").withHeaders(HttpHeaders.`Content-Type`(ContentType(MediaType.custom("application/json"))))).value.get.get)
    assertResponse(200, null, request(get("/headers")).value.get.get, "Headers blew up.")
    assertResponse(200, "* {color:#AAA;}", request(get("/file")).value.get.get)
    assertResponse(200, "file.ending", request(get("/static/file.ending")).value.get.get)
    assertResponse(500, "<h1>An error occurred at", request(get("/crash")).value.get.get, "The crash, should crash successfully.")
  }
